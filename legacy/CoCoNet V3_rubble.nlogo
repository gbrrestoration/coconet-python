...
;      type "draw month = " print draw_month
      ...
      type "Update ts took: " print timer
...
;      monitor-cots-coral
      ...
      type "Coral spawning, " print timer
      ...
      type "Fish spawning, " print timer
...
; Optimization for the recruitment calculations
; returns recruits and new thermal tolerance
to-report calculate-recruitment [c_source thermal_source c_existing thermal_existing rate rate_i limits]
  let recruits random-float (c_source * limits) * (rate / rate_i)
  let new_thermal max (list 1 ((recruits * thermal_source + c_existing * thermal_existing) /
                              (recruits + c_existing + small)))
  report (list recruits new_thermal)
end
...
;;  nw:with-context reefs coral-links
    ...
    [
      ...
      [
        ...
        [
          ...
          ; More efficient recruitment calculations
          let result calculate-recruitment C_sa_source thermal_sa_source C_sa thermal_sa rate_sa rate_sa_i limits
          set C_sa C_sa + item 0 result
          set thermal_sa item 1 result
          set recruits_total recruits_total + item 0 result

          set result calculate-recruitment C_tt_source thermal_tt_source C_tt thermal_tt rate_tt rate_tt_i limits
          set C_tt C_tt + item 0 result
          set thermal_tt item 1 result
          set recruits_total recruits_total + item 0 result

          set result calculate-recruitment C_ta_source thermal_ta_source C_ta thermal_ta rate_ta rate_ta_i limits
          set C_ta C_ta + item 0 result
          set thermal_ta item 1 result
          set recruits_total recruits_total + item 0 result

          set result calculate-recruitment C_mo_source thermal_mo_source C_mo thermal_mo rate_mo rate_mo_i limits
          set C_mo C_mo + item 0 result
          set thermal_mo item 1 result
          set recruits_total recruits_total + item 0 result

          set result calculate-recruitment C_po_source thermal_po_source C_po thermal_po rate_po rate_po_i limits
          set C_po C_po + item 0 result
          set thermal_po item 1 result
          set recruits_total recruits_total + item 0 result

          set result calculate-recruitment C_fa_source thermal_fa_source C_fa thermal_fa rate_fa rate_fa_i limits
          set C_fa C_fa + item 0 result
          set thermal_fa item 1 result
          set recruits_total recruits_total + item 0 result
        ]
      ]
    ]
    ...
    [
      ...
      [
        ...
        [
          ...
          ;; Main recruitment code using the helper function
          let result []
          let new_recruits_total 0

          ;; Process staghorn acropora (sa)
          set result calculate-recruitment C_sa_source thermal_sa_source C_sa thermal_sa rate_sa rate_sa_i limits
          set C_sa C_sa + item 0 result
          set thermal_sa item 1 result
          set recruits_total recruits_total + item 0 result

          ;; Process thermally tolerant (tt)
          set result calculate-recruitment C_tt_source thermal_tt_source C_tt thermal_tt rate_tt rate_tt_i limits
          set C_tt C_tt + item 0 result
          set thermal_tt item 1 result
          set recruits_total recruits_total + item 0 result

          ;; Process tabular acropora (ta)
          set result calculate-recruitment C_ta_source thermal_ta_source C_ta thermal_ta rate_ta rate_ta_i limits
          set C_ta C_ta + item 0 result
          set thermal_ta item 1 result
          set recruits_total recruits_total + item 0 result

          ;; Process montipora (mo)
          set result calculate-recruitment C_mo_source thermal_mo_source C_mo thermal_mo rate_mo rate_mo_i limits
          set C_mo C_mo + item 0 result
          set thermal_mo item 1 result
          set recruits_total recruits_total + item 0 result

          ;; Process poritidae (po)
          set result calculate-recruitment C_po_source thermal_po_source C_po thermal_po rate_po rate_po_i limits
          set C_po C_po + item 0 result
          set thermal_po item 1 result
          set recruits_total recruits_total + item 0 result
  ...
  [
    ...
    if ( C_site + R_site ) > 0.7
    [
      let CR_site ( C_site + R_site + 0.3 )
      set C_fa C_fa / CR_site
      set C_po C_po / CR_site
      set C_mo C_mo / CR_site
      set C_ta C_ta / CR_site
      set C_tt C_tt / CR_site
      set C_sa C_sa / CR_site
    ]
    set C_site C_sa + C_ta + C_mo + C_fa + C_po + C_tt
    if ( C_site + R_site ) > 1.0
    [
